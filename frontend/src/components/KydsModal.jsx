...
// Short labels for the step carousel tabs — full section titles are longer.
...
// and every step up to the furthest visited stays filled as progress.
function StepNav({ activeIndex, maxSeenIndex, onJump }) {
  return (
    <div className="flex flex-shrink-0 items-center gap-1.5 border-b border-line bg-cream px-6 py-2.5">
      {SECTION_LABELS.map((label, i) => {
        ...
        return (
          ...
          >
            <span
              className={`h-1.5 w-full rounded-full transition-all duration-[420ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${
                isCurrent
                  ? 'scale-y-125 bg-teal-deep shadow-[0_0_0_3px_rgba(18,64,62,0.22)]'
                  : visited
                    ? 'bg-teal/50'
                    : 'bg-line'
              }`}
            />
            <span
              className={`text-center text-[10px] font-semibold leading-tight transition-colors duration-[420ms] ease-[cubic-bezier(0.22,1,0.36,1)] ${
                isCurrent ? 'text-teal-deep' : 'text-ink-soft'
              }`}
            >
              {label}
            </span>
...
export default function KydsModal({ onSkip, onSave, initialForm = null, editing = false }) {
  ...
  const lastStep = SECTION_LABELS.length - 1

  const goTo = (i) => {
    const next = Math.max(0, Math.min(lastStep, i))
    if (next === activeIndex) return
    setSlideDir(next > activeIndex ? 1 : -1)
    setActiveIndex(next)
    setMaxSeenIndex((prev) => Math.max(prev, next))
  }

  // Reset vertical scroll when the carousel step changes.
  useEffect(() => {
  ...
  const sectionClass = 'flex w-full min-w-0 flex-col gap-2.5 rounded-[10px] border border-line bg-white px-[18px] pb-[18px] pt-4'
  const panelAnim = slideDir >= 0 ? 'dhara-carousel-panel-forward' : 'dhara-carousel-panel-back'
  ...
  const panels = [
    ...
    <section key="modality" className={sectionClass}>
      <div className="flex items-center gap-2.5">
    ...
    <section key="sensitivity" className={sectionClass}>
      <div className="flex items-center gap-2.5">
    ...
    <section key="access" className={sectionClass}>
      <div className="flex items-center gap-2.5">
    ...
    <section key="granularity" className={sectionClass}>
      <div className="flex items-center gap-2.5">
    ...
    <section key="frequency" className={sectionClass}>
      <div className="flex items-center gap-2.5">
    ...
    <section key="storage" className={sectionClass}>
      <div className="flex items-center gap-2.5">
    ...
    <section key="notes" className={sectionClass}>
      <div className="flex items-center gap-2.5">
  ...
  return mounted
    ...
    <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-[rgba(16,64,63,0.52)] p-5" role="dialog" aria-modal="true" aria-labelledby="kyds-title">
      <div className="flex h-[min(720px,90vh)] w-[min(860px,calc(100vw-2.5rem))] flex-none flex-col overflow-hidden rounded-[14px] bg-surface shadow-dhara">
        ...
        <StepNav activeIndex={activeIndex} maxSeenIndex={maxSeenIndex} onJump={goTo} />

        {/* One step at a time — slide + fade in the direction of travel. */}
        <div
          ref={panelScrollRef}
          className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden px-6 pb-6 pt-[18px]"
        >
          <div
            key={`${activeIndex}-${slideDir}`}
            className={panelAnim}
          >
            {panels[activeIndex]}
          </div>
        ...
        <div className="flex flex-shrink-0 items-center justify-between gap-3 border-t border-line bg-white px-5 py-3">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            {saveError && <span className="truncate text-[12.5px] text-[#b3261e]" title={saveError}>{saveError}</span>}
          </div>
          <div className="flex flex-none items-center gap-3">
            <div className="flex items-center gap-1.5">
              <button
                type="button"
                className="rounded-[7px] border border-line bg-white px-3.5 py-2 text-[13px] font-semibold text-ink hover:bg-cream disabled:cursor-default disabled:opacity-40"
              ...
              <button
                type="button"
                className="rounded-[7px] border border-teal bg-white px-3.5 py-2 text-[13px] font-semibold text-teal hover:bg-sage disabled:cursor-default disabled:opacity-40"
